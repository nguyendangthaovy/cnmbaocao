...
const dateUtils = {
    toTime: (dateString) => {
        ...
        if (date < dateWasMinus7day) return `0${date.getDate()}/${date.getMonth() + 1}`.slice(-2);
    ...
    toTimeSent: (dateString) => {
        const date = new Date(dateString);

        // const now = new Date();
        // const numberMiliseconds = now - date;

        // let hour = Math.floor(numberMiliseconds / HOURSE_MILISECONDS);
        // if (hour > 0) hour = `0${hour}`.slice(-2);
        // let minute = Math.floor(numberMiliseconds / MINUTE_MILISECONDS);
        // if (minute > 0) minute = `0${minute}`.slice(-2);
