...
messageSchema.statics.getByIdAndConversationId = async (_id, conversationId, message = 'Message') => {
...
messageSchema.statics.getListByConversationIdAndUserIdOfGroup = async (conversationId, userId, skip, limit) => {
    const messages = await Message.aggregate([
        {
            $match: {
...
messageSchema.statics.getListByConversationIdAndTypeAndUserId = async (conversationId, type, userId, skip, limit) => {
    const messages = await Message.aggregate([
        {
            $match: {
                conversationId: ObjectId(conversationId),
...
messageSchema.statics.getListByConversationIdAndUserIdOfIndividual = async (conversationId, userId, skip, limit) => {
    const messages = await Message.aggregate([
        {
            $match: {
                conversationId: ObjectId(conversationId),
...
messageSchema.statics.getListFilesByTypeAndConversationId = async (type, conversationId, userId, skip, limit) => {
