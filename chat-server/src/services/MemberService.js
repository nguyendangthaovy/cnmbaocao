...
class MemberService {
    ...
    async leaveGroup(conversationId, userId) {
        ...
        await Conversation.updateOne({ _id: conversationId }, { $pull: { members: userId, managerIds: userId } });
    ...
    async joinConversationFromLink(conversationId, myId) {
        ...
        if (!conversation.type || !conversation.isJoinFromLink)
            throw new UserError('Only group conversation or group not permission join');
        ...
        if (isExistsInConversation) throw new UserError('Exists in conversation');
    ...
    async addManagersForConversation(conversationId, newManagerIds, myId) {
        ...
        if (!type || leaderId + '' !== myId)
            throw new UserError('Add managers failed, not is leader or only conversation group');
        ...
        newManagerIds.forEach((userIdEle) => {
            ...
            if (index === -1 && userIdEle != myId) managerIdsTempt.push(userIdEle);
        });
        ...
        const message = await messageService.updateWhenHasNewMessage(saveMessage, conversationId, myId);
    ...
    async deleteManagersForConversation(conversationId, deleteManagerIds, myId) {
        const conversation = await Conversation.getByIdAndUserId(conversationId, myId);

        const { type, leaderId, managerIds } = conversation;

        if (!type || leaderId + '' !== myId)
            throw new UserError('Delete managers failed, not is leader or only conversation group');
        ...
        await Conversation.updateOne({ _id: conversationId }, { $set: { managerIds: managerIdsTempt } });
